...
function findSuperSpenderEmails(customers:Customer[], orderData: OrderData[]): string[]{
    const superSpenderEmails:string[] = []
    // look through the list of orderData objects
    // create a map of customers and what they've spent
    // take each orderdata and add to customer map the amount spent
    // then look through customerdata and check who as spent at least 100
    // add superspender email to superspenderemail array

    const customerSpending = new Map()
    
    for (const customer of customers){
        if (!customerSpending.has(customer.email)){
            customerSpending.set(customer.email, 0)
        }
        for (const order of orderData){

            if(order.customerId === customer.id){
                let total = customerSpending.get(customer.email) + order.amount
                customerSpending.set(customer.email, total)
            }
            
        }
    }
    console.log(customerSpending)
    for (const email in customerSpending){
        const totalSpendAmount = customerSpending.get(email)
        console.log(totalSpendAmount)
        if(totalSpendAmount > 100){
            superSpenderEmails.push(email)
        }
    }
    

    console.log(superSpenderEmails)
    


    return superSpenderEmails
}

findSuperSpenderEmails(customers, orderData)
findSuperSpenderEmails(customers2, orderData2)